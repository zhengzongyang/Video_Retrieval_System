...
class CLIP2Video_Encoder(CLIP2VideoPreTrainedModel):
    """
    Attributes:
    ...
    def __init__(self, cross_config, clip_state_dict, task_config):
        super(CLIP2Video_Encoder, self).__init__(cross_config)
        ...
        logger.warning("\t embed_dim: {}".format(embed_dim))
        logger.warning("\t image_resolution: {}".format(image_resolution))
        logger.warning("\t vision_layers: {}".format(vision_layers))
        logger.warning("\t vision_width: {}".format(vision_width))
        logger.warning("\t vision_patch_size: {}".format(vision_patch_size))
        logger.warning("\t context_length: {}".format(context_length))
        logger.warning("\t vocab_size: {}".format(vocab_size))
        logger.warning("\t transformer_width: {}".format(transformer_width))
        logger.warning("\t transformer_heads: {}".format(transformer_heads))
        logger.warning("\t transformer_layers: {}".format(transformer_layers))

        cut_top_layer = 0
        logger.warning("\t cut_top_layer: {}".format(cut_top_layer))
        ...
        if hasattr(task_config, "sim_type"):
            ...
            logger.warning("\t sim_type: {}".format(self.sim_type))
    ...
    def get_visual_feature(self, video, video_mask,video_frame=-1):
        video = torch.as_tensor(video).float().to("cuda:0")
        b, pair, bs, ts, channel, h, w = video.shape
        video = video.view(b * pair * bs * ts, channel, h, w)
        video_frame = bs * ts
    ...
    def get_text_feature(self,sequence_output, attention_mask, shaped=False):
    ...
    def get_video_feature(self, visual_output, video_mask, shaped=False):
